...
namespace Corrector.Core.Global {
    public static class DirectoryHelper {
        ...
        public static void Traverse(DirectoryInfo directory, Action<DirectoryInfo> action, string label = "", int depth = int.MaxValue) {
            if (depth-- < 0) return;
            if (directory.GetDirectories() == null) return;
            if (label == "" || directory.FullName.Split('\\').Contains(label)) action(directory);
            //depth--;
