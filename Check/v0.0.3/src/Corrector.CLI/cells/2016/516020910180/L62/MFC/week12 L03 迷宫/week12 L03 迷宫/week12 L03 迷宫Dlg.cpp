...
// week12 L03 迷宫Dlg.cpp : 实现文件
//

#include "stdafx.h"
#include "week12 L03 迷宫.h"
#include "week12 L03 迷宫Dlg.h"
...
Cweek12L03迷宫Dlg::Cweek12L03迷宫Dlg(CWnd* pParent /*=NULL*/)
	: CDialogEx(Cweek12L03迷宫Dlg::IDD, pParent)
	, canvasDC(NULL)
	, m_StartDraw(false)
	, m_StartGame(false)
	, m_ShiftPoint(0)
...
void Cweek12L03迷宫Dlg::DoDataExchange(CDataExchange* pDX)
{
	CDialogEx::DoDataExchange(pDX);
}

BEGIN_MESSAGE_MAP(Cweek12L03迷宫Dlg, CDialogEx)
	ON_WM_SYSCOMMAND()
	ON_WM_PAINT()
	ON_WM_QUERYDRAGICON()
	ON_WM_LBUTTONDOWN()
	ON_WM_LBUTTONUP()
	ON_WM_MOUSEMOVE()
	ON_BN_CLICKED(IDC_BUTTON1, &Cweek12L03迷宫Dlg::OnBnClickedButton1)
	ON_BN_CLICKED(IDC_BUTTON2, &Cweek12L03迷宫Dlg::OnBnClickedButton2)
END_MESSAGE_MAP()


// Cweek12L03迷宫Dlg 消息处理程序

BOOL Cweek12L03迷宫Dlg::OnInitDialog()
{
...
void Cweek12L03迷宫Dlg::OnSysCommand(UINT nID, LPARAM lParam)
{
...
//显示。
HCURSOR Cweek12L03迷宫Dlg::OnQueryDragIcon()
{
...
void Cweek12L03迷宫Dlg::InitCanvas(void)
{
	//设置并更换画刷
...
void Cweek12L03迷宫Dlg::UpdateBox(int rowIndex, int colIndex)
{
	//创建红绿两种画刷
...
void Cweek12L03迷宫Dlg::UpdateBall(int rowMove, int colMove)
{
	if(m_StartGame)
	{
		//更新小球位置
			...
			colIndex>=0 && colIndex<m_colNumOfBoxes)
		{
			...
			**********************/
			...
			{
				choice=MessageBox(TEXT("恭喜到达目的地，游戏结束！r\n是否重新开始？"),
								  TEXT("恭喜你！"),MB_OKCANCEL);
			}
			//游戏失败，弹出对话框
			else if (!boxes[rowIndex][colIndex].m_isSelected)
				choice=MessageBox(TEXT("失败，游戏结束！r\n是否重新开始？"),
...
void Cweek12L03迷宫Dlg::OnLButtonDown(UINT nFlags, CPoint point)
{
...
void Cweek12L03迷宫Dlg::OnLButtonUp(UINT nFlags, CPoint point)
{
	// TODO: 在此添加消息处理程序代码和/或调用默认值
	//关闭开始绘制开关
	m_StartDraw=false;
...
void Cweek12L03迷宫Dlg::OnMouseMove(UINT nFlags, CPoint point)
{
	// TODO: 在此添加消息处理程序代码和/或调用默认值
	//计算当前鼠标所在的方格并更新方格
...
void Cweek12L03迷宫Dlg::OnBnClickedButton1()
{
	// TODO: 在此添加控件通知处理程序代码
	//调用InitCanvas函数初始化游戏
	InitCanvas();
...
void Cweek12L03迷宫Dlg::OnBnClickedButton2()
{
	// TODO: 在此添加控件通知处理程序代码
	//设置并更换画刷
...
BOOL Cweek12L03迷宫Dlg::PreTranslateMessage(MSG* pMsg)
{
	//截获并重写键盘逻辑
