...
void CMazeDlg::OnPaint()
{
	...
	else
	{
		...
		ball = new Ball();
		ball->m_rowIndex = 0;
		ball->m_colIndex = 0;     //  Is this right?
		...
		for(int i=0 ; i<m_rowNumOfBoxes;i++)
	{
		...
		for(int j=0 ; j<m_colNumOfBoxes;j++)
		{
			...
			boxes[i][j].bottom = (1+i)*m_scaleOfBox+2;
			boxes[i][j].left = j*m_scaleOfBox+1;
			boxes[i][j].right = (1+j)*m_scaleOfBox+2;
		}
	}
