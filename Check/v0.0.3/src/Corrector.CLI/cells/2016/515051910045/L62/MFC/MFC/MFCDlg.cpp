...
BEGIN_MESSAGE_MAP(CMFCDlg, CDialogEx)
	...
	ON_STN_CLICKED(IDC_CANVAS, &CMFCDlg::OnStnClickedCanvas)
	ON_WM_LBUTTONDOWN()
	ON_WM_LBUTTONUP()
	ON_WM_MOUSEMOVE()
...
void CMFCDlg::UpdateBox(int rowIndex, int colIndex)
{
	//创建红绿两种画刷
	CBrush rBrush(RGB(255,0,0));
	CBrush gBrush(RGB(0,255,0));
	//判断方格是否有效
	//若有效，则设置方格状态为选中并填充相应的颜色
...
void CMFCDlg::UpdateBall(int rowMove, int colMove)
{
	if(m_StartGame){
		//更新小球位置
...
void CMFCDlg::OnStnClickedCanvas()
{
	// TODO: 在此添加控件通知处理程序代码
}
