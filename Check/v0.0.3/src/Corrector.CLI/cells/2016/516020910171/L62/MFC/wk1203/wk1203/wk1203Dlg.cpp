...
Cwk1203Dlg::Cwk1203Dlg(CWnd* pParent /*=NULL*/)
	: CDialogEx(Cwk1203Dlg::IDD, pParent)
	, canvasDC(NULL)
	, m_StartDraw(false)
	, m_StartGame(false)
	, m_ShiftPoint(0)
...
void Cwk1203Dlg::DoDataExchange(CDataExchange* pDX)
{
	CDialogEx::DoDataExchange(pDX);
}

BEGIN_MESSAGE_MAP(Cwk1203Dlg, CDialogEx)
	ON_WM_SYSCOMMAND()
	ON_WM_PAINT()
	ON_WM_QUERYDRAGICON()
	ON_WM_LBUTTONDOWN()
	ON_WM_LBUTTONUP()
	ON_WM_MOUSEMOVE()
	ON_BN_CLICKED(IDC_BUTTON1, &Cwk1203Dlg::OnBnClickedButton1)
	ON_BN_CLICKED(IDC_BUTTON2, &Cwk1203Dlg::OnBnClickedButton2)
END_MESSAGE_MAP()


// Cwk1203Dlg 消息处理程序

BOOL Cwk1203Dlg::OnInitDialog()
{
...
void Cwk1203Dlg::OnSysCommand(UINT nID, LPARAM lParam)
{
...
void Cwk1203Dlg::OnPaint()
{
	...
	else
	{
		...
		for(int i=0;i<m_rowNumOfBoxes;i++){
			...
			for(int j=1;j<m_colNumOfBoxes;j++){
...
//显示。
HCURSOR Cwk1203Dlg::OnQueryDragIcon()
{
...
void Cwk1203Dlg::InitCanvas(void)
{
	//设置并更换画刷
...
void Cwk1203Dlg::UpdateBox(int rowIndex, int colIndex)
{
	//创建红绿两种画刷
	...
	if (rowIndex>=0&&rowIndex<m_rowNumOfBoxes&&
		colIndex>=0&&colIndex<m_colNumOfBoxes){
			...
			else canvasDC->FillRect(boxes[rowIndex][colIndex].GetContent(),&rBursh);
	}
}



void Cwk1203Dlg::UpdateBall(int rowMove, int colMove)
{
	if(m_StartGame){
		//更新小球位置
		...
		colIndex>=0&&colIndex<m_colNumOfBoxes){
			...
			if (rowIndex==m_rowNumOfBoxes-1&&colIndex==m_colNumOfBoxes-1){
				choice=MessageBox(TEXT("成功到达目的地！\r\n是否重新开始？"),
...
void Cwk1203Dlg::OnLButtonDown(UINT nFlags, CPoint point)
{
...
void Cwk1203Dlg::OnLButtonUp(UINT nFlags, CPoint point)
{
	// TODO: 在此添加消息处理程序代码和/或调用默认值
	//关闭开始绘制的开关
	m_StartDraw=false;
...
void Cwk1203Dlg::OnMouseMove(UINT nFlags, CPoint point)
{
	// TODO: 在此添加消息处理程序代码和/或调用默认值
	//计算当前鼠标所在的方格并更新方格
...
void Cwk1203Dlg::OnBnClickedButton1()
{
	// TODO: 在此添加控件通知处理程序代码
	//调用InitCanvas函数初始化游戏
	InitCanvas();
...
void Cwk1203Dlg::OnBnClickedButton2()
{
	// TODO: 在此添加控件通知处理程序代码
	//设置并更换画刷
...
BOOL Cwk1203Dlg::PreTranslateMessage(MSG* pMsg)
{
	//截获并重写键盘逻辑
