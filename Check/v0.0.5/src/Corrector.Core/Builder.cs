...
namespace Corrector.Core
{
    public class Builder
    {
        public struct Compiler {
            ...
            FileInfo Bin { get; }
            List<DirectoryInfo> IncludePaths { get; }

            public Compiler(DirectoryInfo rootPath, FileInfo binPath, List<DirectoryInfo> includes) {
                Root = rootPath;
                Bin = binPath;
        ...
        public struct Linker {
            DirectoryInfo Root { get; }
            FileInfo Bin { get; }
            List<DirectoryInfo> LibraryPaths { get; }

            public Linker(DirectoryInfo rootPath, FileInfo binPath, List<DirectoryInfo> libraries) {
                Root = rootPath;
                Bin = binPath;
        ...
        public async Task<CompileResult> Build(DirectoryInfo workDirectory) {
            ...
            var toolPath = ConfigInfo.ToolPath.FullName;
        ...
        private async Task<CompileResult> Compile(FileInfo projectPath, string parameters)
        {
            ...
            process.StartInfo.FileName = ConfigInfo.CompilerPath.GetSubfile(@"");
