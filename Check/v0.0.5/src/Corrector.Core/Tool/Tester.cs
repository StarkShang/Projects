...
namespace Corrector.Core
{
    ...
    public class Tester {
        static public DirectoryInfo TestRoot { get; set; }
        ...
        public void Correct(FileInfo executableFile, Recorder logger)
        {
            if (executableFile == null) logger.Log("Cannot file executable file");
            var testPath = Path.Combine(Tester.TestRoot.FullName, executableFile.Directory.Parent.Name, Regex.Match(executableFile.Directory.Name, @"L\d{2}").Value);
