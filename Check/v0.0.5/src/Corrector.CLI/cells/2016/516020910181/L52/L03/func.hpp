...
{
	int data;                               //用data来存储数据
	LinkedNode* prev;                       //创建指向前一个结构体的指针
	LinkedNode* next;                       //创建指向后一个结构体的指针
};

LinkedNode* create(int* array, int size)    //创建create函数
{
	...
	for (int i = 1; i < size; i++)          //循环结构 给每一个结构体赋值
	{
		iterator->next = new LinkedNode();  //创建一个新的结构体，即现有结构体的后一个结构体 
		iterator->next->data = array[i];    //给这个新的结构体赋值
		iterator->next->prev = iterator;    //这个新的结构体的prev指针指向现有的最后一个结构体
		iterator->next->next = nullptr;     //先使的这个结构体的next指针指向空
