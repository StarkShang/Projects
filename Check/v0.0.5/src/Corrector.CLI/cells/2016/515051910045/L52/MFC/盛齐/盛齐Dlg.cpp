...
C盛齐Dlg::C盛齐Dlg(CWnd* pParent /*=NULL*/)
	: CDialogEx(C盛齐Dlg::IDD, pParent)
	, m_StartDraw(false)
	, m_PrevPoint(0)
	, m_ShiftPoint(0)
{
...
void C盛齐Dlg::DoDataExchange(CDataExchange* pDX)
{
	CDialogEx::DoDataExchange(pDX);
}

BEGIN_MESSAGE_MAP(C盛齐Dlg, CDialogEx)
	ON_WM_SYSCOMMAND()
	ON_WM_PAINT()
	ON_WM_QUERYDRAGICON()
	ON_WM_LBUTTONDOWN()
	ON_WM_LBUTTONUP()
	ON_WM_MOUSEMOVE()
	ON_BN_CLICKED(IDC_BUTTON1, &C盛齐Dlg::OnBnClickedButton1)
END_MESSAGE_MAP()


// C盛齐Dlg 消息处理程序

BOOL C盛齐Dlg::OnInitDialog()
{
...
void C盛齐Dlg::OnSysCommand(UINT nID, LPARAM lParam)
{
...
//显示。
HCURSOR C盛齐Dlg::OnQueryDragIcon()
{
...
void C盛齐Dlg::OnLButtonDown(UINT nFlags, CPoint point)
{
...
void C盛齐Dlg::OnLButtonUp(UINT nFlags, CPoint point)
{
	// TODO: 在此添加消息处理程序代码和/或调用默认值
	m_StartDraw=false;
...
void C盛齐Dlg::OnMouseMove(UINT nFlags, CPoint point)
{
	// TODO: 在此添加消息处理程序代码和/或调用默认值
	if(m_StartDraw)
	{
...
void C盛齐Dlg::OnBnClickedButton1()
{
	// TODO: 在此添加控件通知处理程序代码
	CWnd *pCanvasWnd=GetDlgItem(IDC_CANVAS);
	CDC *pCanvasDC=pCanvasWnd->GetDC();
